"""
Phase 6N — Re-evaluate on GroceryDataset (Triplet Loss ResNet50)
==============================================================
Runs the exact same evaluation as Phase 6F/6K, but uses the
target-domain metric learning ResNet50 backbone (Phase 6M) instead of
...
METRIC_OUTPUT_DIR = BASE_DIR / "outputs" / "metric_learning"
MODEL_PATH = METRIC_OUTPUT_DIR / "resnet50_triplet_best.pth"
...
OUTPUT_DIR = BASE_DIR / "outputs" / "retrieval_metric"
...
def load_metric_backbone():
    print("\n📦 Loading Triplet-Loss ResNet50 backbone...")
    ...
    model.eval()
    model.to(DEVICE)
    print("  ✅ Triplet-Loss backbone loaded successfully")
    return model
...
def plot_metrics(correct_sims, incorrect_sims, per_class, save_prefix):
    ...
    ax.set_ylabel('Count')
    ax.set_title('Similarity Distribution (Triplet Loss)')
    ax.legend()
    ...
    ax.set_title('Per-Brand Retrieval Accuracy (Triplet Loss)')
    ax.set_ylim(0, 1.05)
...
def main():
    print("=" * 70)
    print("  PHASE 6N: RE-EVALUATE ON GROCERYDATASET (TRIPLET LOSS)")
    print("=" * 70)
    ...
    print(f"\n  RESULTS: TRIPLET-LOSS EMBEDDINGS (Target Domain)")
    ...
    plot_metrics(correct_sims, incorrect_sims, per_class, str(OUTPUT_DIR / "metric"))
    print(f"\n  Saved plots to: {OUTPUT_DIR}")

    # 3. YOLO Crop Retrieval
    print("\n" + "─" * 70)
    ...
    eval_output = {
        "model": "ResNet50 (Triplet Loss on GroceryDataset)",
